...
export default function Paso1() {
  ...
  return (
    <View style={styles.contenedor}>
      <Text style={styles.titulo}>Conexion al Sistema de Ventas</Text>

      <View style={[styles.tarjeta, { borderColor: colorEstado }]}>
        <Text style={[styles.estado, { color: colorEstado }]}>{estadoTexto}</Text>
        <Text style={styles.detalle}>servidor-ventas · 192.168.1.100:3000</Text>
      </View>
      ...
      >
        <Text style={styles.textoBoton}>
          {conectado ? 'Conectado' : 'Reintentar conexion'}
        </Text>
      </Pressable>

      <Pressable style={styles.botonSecundario} onPress={reiniciar}>
        <Text style={styles.textoSecundario}>Reiniciar simulacion</Text>
      </Pressable>
    </View>
  );
}
