...
export default function Paso7() {
  ...
  const { estado, intentos, latencia, reconectar, reiniciar } =
    useConexionSimulada('Inventario Principal')
  ...
  useEffect(() => {
    ...
    const intervalo = setInterval(() => {
      ...
      const eventos = [
        `[${ahora}] Consulta de inventario completada en ${Math.floor(Math.random() * 50) + 5}ms`,
        `[${ahora}] Nueva moto agregada al catálogo`,
        `[${ahora}] Precio actualizado: Honda CBR 600RR`,
        `[${ahora}] Stock sincronizado: 3 unidades disponibles`,
      ]
    ...
    <View style={[styles.contenedor, { backgroundColor: colores.fondo }]}>
      ...
      <View style={[
        ...
        <Text style={[styles.detalle, { color: colores.detalle }]}>
          192.168.1.100 · API REST · Puerto 3000
        </Text>
